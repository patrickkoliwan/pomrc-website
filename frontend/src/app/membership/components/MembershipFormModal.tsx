...
const Payment = dynamic(() => import('./Payment'), { // Assuming path relative to this component
  ssr: false,
  loading: () => <div className="p-6 text-center text-dark-teal">Loading payment section...</div>,
});

// Validation schema (moved from page.tsx)
...
const MembershipFormModal: React.FC<MembershipFormModalProps> = ({
  ...
  const onSubmit = async (data: MembershipFormData) => {
    try {
      ...
      alert("Application submitted successfully! (Data logged to console)"); // Placeholder feedback
      onClose(); // Close the modal on successful submission
    } catch (error) {
      console.error("Submission failed:", error);
      alert("Submission failed. Please try again."); // Placeholder error feedback
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4">
      {/* Modal Content Box */}
      <div className="bg-light-cream rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 text-gray-500 hover:text-deep-red z-50 text-3xl leading-none font-semibold p-1"
        ...
        <div className="p-6 sm:p-8">
          <h1 className="text-3xl font-bold text-dark-teal mb-6 text-center">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end pt-4">
              <button
                type="submit"
                disabled={isLoading}
                className="px-6 py-3 bg-dark-teal text-white rounded-md hover:bg-muted-teal focus:outline-none focus:ring-2 focus:ring-dark-teal focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
